...
public class ArchiveProjectAsyncTest
{
    // Archive a project successfully when all conditions are met
    [Fact]
    public async Task WhenAllIsRight_ReturnSuccess()
    {
        ...
        _companyUserRepository.Setup(companyRepository => companyRepository.AnyAsync(
        ...
        // Act
        var result = await projectService.ArchiveProjectAsync(userId, project.Id);
    ...
    // Return an error when the project to archive is not found
    [Fact]
    public async Task WhenProjectToArchiveNotFound_ReturnAnError()
    {
        ...
        // Act
        var result = await projectService.ArchiveProjectAsync(userId, projectId);
    ...
    // Return an error when the project to archive is already archived
    [Fact]
    public async Task WhenProjectToArchiveAlreadyArchived_ReturnAnAlreadyArchivedError()
    {
        var projectService = ServiceUtilities.CreateProjectServices(
            out _,
            out Mock<IAsyncRepository<CompanyUser>> _companyUserRepository,
        ...
        // Assert
        Assert.False(result.IsSuccess);
        Assert.Contains(ErrorStrings.RegisterAlreadyArchived, result.Errors.First().ToString());
    }
