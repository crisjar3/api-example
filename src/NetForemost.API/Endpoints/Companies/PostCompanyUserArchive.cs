...
public class PostCompanyUserArchive : EndpointBaseAsync.WithRequest<PostCompanyUserArchiveRequest>.WithActionResult<CompanyUserDto>
{
    protected readonly ICompanyUserService _companyUserService;
    protected readonly ILogger<PostCompanyUserArchive> _logger;
    protected readonly IMapper _mapper;
    private readonly UserManager<User> _userManager;

    public PostCompanyUserArchive(
        ICompanyUserService companyUserService,
        ILogger<PostCompanyUserArchive> logger,
        IMapper mapper,
    ...
    [HttpPost(Routes.Company.PostCompanyUserArchive)]
    [SwaggerOperation(
                Summary = "Archives a company user.",
                Description = "Set a company user as archived to true.",
                OperationId = "Companies.PostCompanyUserArchived",
                Tags = new[] { "Company" })
            ]
    [Authorize]
    public override async Task<ActionResult<CompanyUserDto>> HandleAsync(PostCompanyUserArchiveRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var result = await _companyUserService.ArchiveCompanyUserAsync(user.Id, request.CompanyUserId); ;
