...
public class GetGoalCompletedLateByUserIdAndDateRange : BaseSpecificationPagination<Goal, GetLateGoalDto>
{
    public GetGoalCompletedLateByUserIdAndDateRange(string userId, DateTime from, DateTime to, int pageNumber, int perPage, bool paginate, double timeZone) : base(pageNumber, perPage, paginate)
    {
        Query.Include(goal => goal.Owner).ThenInclude(owner => owner.CompanyUser).ThenInclude(companyUser => companyUser.User);

        Query.Where(goal => goal.Owner.CompanyUser.UserId == userId && goal.ActualEndDate != null && goal.ActualEndDate > goal.TargetEndDate);
        ...
        Query.Select(goal => new GetLateGoalDto
        {
            Id = goal.Id,
            Name = goal.Name,
